...
class Admin_Peminjaman extends CI_Controller{
    ...
    function index($offset=0,$order_column='id_pinjaman',$order_type='asc'){
        if(empty($offset)) $offset=0;
        if(empty($order_column)) $order_column='id_pinjaman';
        if(empty($order_type)) $order_type='asc';
        
        //load data
        $data['angsuran']=$this->m_admin_peminjaman->semua($this->limit,$offset,$order_column,$order_type)->result();
        $data['title']="Daftar Permohonan Pinjaman";
        
        $config['base_url']=site_url('admin_peminjaman/index/');
        $config['total_rows']=$this->m_admin_peminjaman->jumlah();
        ...
        if($this->uri->segment(3)=="delete_success")
            ...
            $data['message']='';
            $this->template->display('admin_peminjaman/index',$data);
    }
